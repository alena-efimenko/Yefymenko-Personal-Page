...
Running time of KMP: 0.8171639000000002
  ...
  This exercise asks to describe an implementation of $RANDOM(a, b)$ procedure that only makes calls to $RANDOM(0, 1)$. <br/>
  A definition of $RANDOM(a, b)$ in the
  book is that the procedure returns an integer between a and b, inclusive, where each integer being equally likely. <br/>
...
def next = scala.util.Random.nextBoolean
    ...
    <br/>
    Suppose we have a=1 and b=8 as arguments fed into $RANDOM(a, b)$, the total number of integers in the range is power of two just for convenience.
    <br/>
    ...
    we should expect a random result from 1 to 8 with probability $ p = (1/2)^3 = 1/8$.
    <br/>
    And it is what we actually need to solve the exercise:
    <br/>
    <br/>
    <div j-scala-decorator>
      <code>
...
private def log2(i : Int) = math.log(i)/math.log(2)
    ...
    <br/>
      But this solution works only if the total number of integers to generate equals to power of two.
      <br/>
      And here is a fix for that:
      <br/>
      let's generate numbers within a range $[1, 2^{\lceil\ log_2(n) \rceil}]$ while generated integer is not in the input range, where
      "n" is the second argument of the procedure.
      <br/>
      Then the returned integers will have equal probability and be within the input range.
    <br/>
    <br/>
    <div j-scala-decorator>
      <code>
